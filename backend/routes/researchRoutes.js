...
router.post('/team', authenticateToken, async (req, res) => {
  try {
    ...
    const researchCollection = await db.collection('researches')
    const researchLeader = await researchCollection.findOne({
      _id: new ObjectId(req.body.researchId),
      submittedBy: new ObjectId(req.user.id),
    })
    const research = await researchCollection.findOne({
      _id: new ObjectId(req.body.researchId),
    })

    if (!research) {
      res.status(404).json({ message: 'Research not found' })
    } else if (existingTeam || researchLeader) {
